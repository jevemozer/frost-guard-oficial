...
export const LayoutContent = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className={`flex min-h-screen bg-background text-foreground`}>
      ...
      {/* Renderiza o Sidebar se o usuário estiver logado e na página correta */}
      <main className={`flex-1 p-4 ${shouldRenderSidebar ? 'pl-4 pt-10' : ''}`}>
        {' '}
        {/* Adiciona padding se o Sidebar estiver visível */}
        {children}
      </main>
    </div>
  );
